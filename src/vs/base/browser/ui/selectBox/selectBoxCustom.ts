...
export class SelectBoxList extends Disposable implements ISelectBoxDelegate, IListVirtualDelegate<ISelectOptionItem> {
	...
	private layoutSelectDropDown(preLayoutPosition?: boolean): boolean {
		...
		if (this.selectList) {
			...
			const selectOptimalWidth = this.selectBoxOptions.customDrawnDropdownWidth ? `${this.selectBoxOptions.customDrawnDropdownWidth}px` : Math.max(selectMinWidth, Math.round(selectWidth)).toString() + 'px';
