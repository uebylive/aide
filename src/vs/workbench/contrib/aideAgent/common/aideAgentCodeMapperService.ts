...
import { IChatAgentResult } from './aideAgentAgents.js';
import { IChatResponseModel } from './aideAgentModel.js';
import { IChatContentReference } from './aideAgentService.js';
...
export const IAideAgentCodeMapperService = createDecorator<IAideAgentCodeMapperService>('aideAgentcodeMapperService');

export interface IAideAgentCodeMapperService {
...
export class CodeMapperService implements IAideAgentCodeMapperService {
	...
	async mapCodeFromResponse(responseModel: IChatResponseModel, response: ICodeMapperResponse, token: CancellationToken) {
		...
		/*
		// TODO(@ghostwriternr): This is inherited from the chat implementation, but is broken because
		// they have the notion of a response, which we don't anymore. Look at this if/when we start
		// using this method.
		for (const exchange of responseModel.session.getExchanges()) {
