...
import { CONTEXT_LANGUAGE_MODELS_ARE_USER_SELECTABLE } from './aideAgentContextKeys.js';
...
export const IAideAgentLMService = createDecorator<IAideAgentLMService>('IAideAgentLMService');
...
export interface IAideAgentLMService {
...
export const languageModelExtensionPoint = ExtensionsRegistry.registerExtensionPoint<IUserFriendlyLanguageModel | IUserFriendlyLanguageModel[]>({
	...
	activationEventsGenerator: (contribs: IUserFriendlyLanguageModel[], result: { push(item: string): void }) => {
		for (const contrib of contribs) {
			result.push(`onAideAgentLMChat:${contrib.vendor}`);
		}
	}
});

export class LanguageModelsService implements IAideAgentLMService {
	...
	async selectLanguageModels(selector: ILanguageModelChatSelector): Promise<string[]> {
		...
		if (selector.vendor) {
			...
			await this._extensionService.activateByEvent(`onAideAgentLMChat:${selector.vendor}}`);
		} else {
			// activate all extensions that do language models
			const all = Array.from(this._vendors).map(vendor => this._extensionService.activateByEvent(`onAideAgentLMChat:${vendor}`));
