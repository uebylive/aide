...
export const IAideAgentLMToolsService = createDecorator<IAideAgentLMToolsService>('IAideAgentLMToolsService');

export type CountTokensCallback = (input: string, token: CancellationToken) => Promise<number>;

export interface IAideAgentLMToolsService {
...
export class LanguageModelToolsService extends Disposable implements IAideAgentLMToolsService {
	_serviceBrand: undefined;
	...
	async invokeTool(dto: IToolInvocation, countTokens: CountTokensCallback, token: CancellationToken): Promise<IToolResult> {
		...
		if (!tool.impl) {
			await this._extensionService.activateByEvent(`onAideAgentLMTool:${dto.toolId}`);
