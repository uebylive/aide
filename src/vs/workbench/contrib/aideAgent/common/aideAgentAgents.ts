...
export const IAideAgentAgentService = createDecorator<IAideAgentAgentService>('aideAgentAgentService');
...
export interface IAideAgentAgentService {
	...
	initSession(id: string, sessionId: string): void;
	invokeAgent(agent: string, request: IChatAgentRequest, token: CancellationToken): Promise<IChatAgentResult>;
...
export class ChatAgentService implements IAideAgentAgentService {
	...
	initSession(id: string, sessionId: string): void {
		...
		data.impl.initSession(sessionId);
	}

	async invokeAgent(id: string, request: IChatAgentRequest, token: CancellationToken): Promise<IChatAgentResult> {
		const data = this._agents.get(id);
		if (!data?.impl) {
			throw new Error(`No activated agent with id "${id}"`);
		}
...
export class MergedChatAgent implements IChatAgent {
	...
	initSession(sessionId: string): void {
		return this.impl.initSession(sessionId);
	}

	async invoke(request: IChatAgentRequest, token: CancellationToken): Promise<IChatAgentResult> {
		return this.impl.invoke(request, token);
	}
...
export const IAideAgentAgentNameService = createDecorator<IAideAgentAgentNameService>('aideAgentAgentNameService');
...
export interface IAideAgentAgentNameService {
	_serviceBrand: undefined;
	getAgentNameRestriction(chatAgentData: IChatAgentData): boolean;
}

export class ChatAgentNameService implements IAideAgentAgentNameService {
