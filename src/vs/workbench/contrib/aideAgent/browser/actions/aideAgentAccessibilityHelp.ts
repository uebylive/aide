...
import { CONTEXT_IN_CHAT_SESSION, CONTEXT_RESPONSE, CONTEXT_REQUEST, CONTEXT_CHAT_LOCATION } from '../../common/aideAgentContextKeys.js';
...
export function getAccessibilityHelpText(type: 'panelChat' | 'inlineChat'): string {
	...
	if (type === 'panelChat') {
		...
		content.push(localize('workbench.action.chat.newChat', 'To create a new session, invoke the New Session command{0}.', '<keybinding:workbench.action.chat.new>'));
	} else {
