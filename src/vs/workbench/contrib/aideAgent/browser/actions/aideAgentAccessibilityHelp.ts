...
import { CONTEXT_CHAT_LOCATION, CONTEXT_IN_CHAT_SESSION, CONTEXT_REQUEST, CONTEXT_RESPONSE } from '../../common/aideAgentContextKeys.js';
...
export function getAccessibilityHelpText(type: 'panelChat' | 'inlineChat', keybindingService: IKeybindingService): string {
	const content = [];
	if (type === 'panelChat') {
...
export function getChatAccessibilityHelpProvider(accessor: ServicesAccessor, editor: ICodeEditor | undefined, type: 'panelChat' | 'inlineChat') {
	...
	return new AccessibleContentProvider(
		type === 'panelChat' ? AccessibleViewProviderId.PanelChat : AccessibleViewProviderId.InlineChat,
...
function getChatFocusKeybindingLabel(keybindingService: IKeybindingService, type: 'panelChat' | 'inlineChat', focusInput?: boolean): string | undefined {
	let kbs;
