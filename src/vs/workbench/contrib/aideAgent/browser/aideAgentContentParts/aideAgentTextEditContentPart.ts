...
const IAideAgentCodeCompareModelService = createDecorator<IAideAgentCodeCompareModelService>('IAideAgentCodeCompareModelService');

interface IAideAgentCodeCompareModelService {
...
export class ChatTextEditContentPart extends Disposable implements IChatContentPart {
	...
	constructor(
		...
		currentWidth: number,
		@IAideAgentCodeCompareModelService private readonly codeCompareModelService: IAideAgentCodeCompareModelService
	) {
		super();
...
export class DiffEditorPool extends Disposable {
	...
	constructor(
		...
		this._pool = this._register(new ResourcePool(() => {
			return instantiationService.createInstance(CodeCompareBlockPart, options, MenuId.AideAgentCompareBlock, delegate, overflowWidgetsDomNode);
		}));
	}
...
class CodeCompareModelService implements IAideAgentCodeCompareModelService {
	...
	async createModel(element: IChatResponseViewModel, chatTextEdit: IChatTextEditGroup): Promise<IReference<{ originalSha1: string; original: IResolvedTextEditorModel; modified: IResolvedTextEditorModel }>> {
		...
		const modified = await this.textModelService.createModelReference((this.modelService.createModel(
			...
			URI.from({ scheme: Schemas.vscodeAideAgentCodeBlock, path: chatTextEdit.uri.path, query: generateUuid() }),
			false
		)).uri);
...
registerSingleton(IAideAgentCodeCompareModelService, CodeCompareModelService, InstantiationType.Delayed);
