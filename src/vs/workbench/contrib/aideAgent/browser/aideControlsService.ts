...
export class AideControlsService extends Disposable implements IAideControlsService {
	...
	*/
	...
	acceptInput(): void {
		if (this._controls) {
			this._controls.widget.acceptInput();
		}
	}

	focusInput(): void {
		if (this._controls) {
			this._controls.widget.focusInput();
		}
	}

	blurInput(): void {
		// TODO(@ghostwriternr): This logic is incorrect. We ideally should go back to the editor the user was last focused on.
		if (this._controls) {
