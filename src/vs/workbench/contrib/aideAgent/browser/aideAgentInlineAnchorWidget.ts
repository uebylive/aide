...
import { chatAttachmentResourceContextKey, hookUpSymbolAttachmentDragAndContextMenu } from '../../chat/browser/chatContentParts/chatAttachmentsContentPart.js';
import { IChatMarkdownAnchorService } from '../../chat/browser/chatContentParts/chatMarkdownAnchorService.js';
import { ExplorerFolderContext } from '../../files/common/files.js';
import { IWorkspaceSymbol } from '../../search/common/search.js';
import { IChatContentInlineReference } from '../common/aideAgentService.js';
...
export class InlineAnchorWidget extends Disposable {
	...
	constructor(
		...
		if (this.data.kind === 'symbol') {
			...
			this._store.add(instantiationService.invokeFunction(accessor => hookUpSymbolAttachmentDragAndContextMenu(accessor, element, contextKeyService, { value: symbol.location, name: symbol.name, kind: symbol.kind }, MenuId.AideAgentInlineSymbolAnchorContext)));
		} else {
			location = this.data;
			...
			this._register(dom.addDisposableListener(element, dom.EventType.CONTEXT_MENU, async domEvent => {
				...
				contextMenuService.showContextMenu({
					...
					getAnchor: () => event,
					getActions: () => {
						const menu = menuService.getMenuActions(MenuId.AideAgentInlineResourceAnchorContext, contextKeyService, { arg: location.uri });
...
registerAction2(class AddFileToChatAction extends Action2 {
	...
	static readonly id = 'aideAgent.inlineResourceAnchor.addFileToChat';
	...
	constructor() {
		super({
			...
			menu: [{
				id: MenuId.AideAgentInlineResourceAnchorContext,
				group: 'chat',
				order: 1,
...
registerAction2(class CopyResourceAction extends Action2 {
	...
	static readonly id = 'aideAgent.inlineResourceAnchor.copyResource';
...
registerAction2(class OpenToSideResourceAction extends Action2 {
	...
	static readonly id = 'aideAgent.inlineResourceAnchor.openToSide';
	...
	constructor() {
		super({
			...
			menu: [MenuId.AideAgentInlineSymbolAnchorContext, MenuId.AideAgentInputSymbolAttachmentContext].map(id => ({
				id: id,
				group: 'navigation',
				order: 1
			}))
...
registerAction2(class GoToDefinitionAction extends Action2 {
	...
	static readonly id = 'aideAgent.inlineSymbolAnchor.goToDefinition';
...
registerAction2(class GoToTypeDefinitionsAction extends Action2 {
	...
	static readonly id = 'aideAgent.inlineSymbolAnchor.goToTypeDefinitions';
...
registerAction2(class GoToImplementations extends Action2 {
	...
	static readonly id = 'aideAgent.inlineSymbolAnchor.goToImplementations';
...
registerAction2(class GoToReferencesAction extends Action2 {
	...
	static readonly id = 'aideAgent.inlineSymbolAnchor.goToReferences';
