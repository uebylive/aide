...
import { IExtensionService, isProposedApiEnabled } from '../../../../workbench/services/extensions/common/extensions.js';
...
export interface ILanguageModelChat {
	...
	sendChatRequest(messages: IAideChatMessage[], from: ExtensionIdentifier, options: { [name: string]: any }, token: CancellationToken): Promise<ILanguageModelChatResponse>;
...
export interface ILanguageModelsService {
	...
	sendChatRequest(identifier: string, from: ExtensionIdentifier, messages: IAideChatMessage[], options: { [name: string]: any }, token: CancellationToken): Promise<ILanguageModelChatResponse>;
...
const languageModelType: IJSONSchema = {
	...
	properties: {
		vendor: {
			type: 'string',
			description: localize('vscode.extension.contributes.AIModels.vendor', "A globally unique vendor of language models.")
		}
	}
};
...
export const languageModelExtensionPoint = ExtensionsRegistry.registerExtensionPoint<IUserFriendlyLanguageModel | IUserFriendlyLanguageModel[]>({
	...
	jsonSchema: {
		description: localize('vscode.extension.contributes.aiModels', "Contribute language models of a specific vendor."),
		oneOf: [
			languageModelType,
			{
				type: 'array',
	...
	activationEventsGenerator: (contribs: IUserFriendlyLanguageModel[], result: { push(item: string): void }) => {
		for (const contrib of contribs) {
			result.push(`onAIModelChat:${contrib.vendor}`);
		}
	}
});
...
export class LanguageModelsService implements ILanguageModelsService {
		...
		this._store.add(languageModelExtensionPoint.setHandler((extensions) => {
			...
			for (const extension of extensions) {
				...
				if (!isProposedApiEnabled(extension.description, 'chatProvider')) {
					extension.collector.error(localize('vscode.extension.contributes.AIModels.chatProviderRequired', "This contribution point requires the 'chatProvider' proposal."));
					continue;
				}
	...
	async sendChatRequest(identifier: string, from: ExtensionIdentifier, messages: IAideChatMessage[], options: { [name: string]: any }, token: CancellationToken): Promise<ILanguageModelChatResponse> {
