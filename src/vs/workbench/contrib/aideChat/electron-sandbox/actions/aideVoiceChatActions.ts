...
import { IChatExecuteActionContext } from 'vs/workbench/contrib/aideChat/browser/actions/aideChatExecuteActions';
import { IChatWidget, IAideChatWidgetService, IQuickChatService, showChatView } from 'vs/workbench/contrib/aideChat/browser/aideChat';
import { AideChatAgentLocation, IAideChatAgentService } from 'vs/workbench/contrib/aideChat/common/aideChatAgents';
import { CONTEXT_CHAT_REQUEST_IN_PROGRESS, CONTEXT_IN_CHAT_INPUT, CONTEXT_CHAT_ENABLED, CONTEXT_RESPONSE, CONTEXT_RESPONSE_FILTERED } from 'vs/workbench/contrib/aideChat/common/aideChatContextKeys';
import { KEYWORD_ACTIVIATION_SETTING_ID } from 'vs/workbench/contrib/aideChat/common/aideChatService';
import { isResponseVM } from 'vs/workbench/contrib/aideChat/common/aideChatViewModel';
import { IVoiceChatService, VoiceChatInProgress as GlobalVoiceChatInProgress } from 'vs/workbench/contrib/aideChat/common/voiceChatService';
import { IExtensionsWorkbenchService } from 'vs/workbench/contrib/extensions/common/extensions';
import { InlineChatController } from 'vs/workbench/contrib/inlineAideChat/browser/inlineChatController';
import { CTX_INLINE_CHAT_FOCUSED } from 'vs/workbench/contrib/inlineAideChat/common/inlineChat';
...
class VoiceChatSessionControllerFactory {
	...
	private static doCreateForFocusedChat(terminalService: ITerminalService, chatWidgetService: IAideChatWidgetService, layoutService: IWorkbenchLayoutService): IVoiceChatSessionController | undefined {
		...
		if (chatWidget?.hasInputFocus()) {
			...
			if (layoutService.hasFocus(Parts.EDITOR_PART)) {
				context = chatWidget.location === AideChatAgentLocation.Panel ? 'editor' : 'inline';
			} else if (
...
export class VoiceChatInChatViewAction extends VoiceChatWithHoldModeAction {
	...
	static readonly ID = 'workbench.action.aideChat.voiceChatInChatView';

	constructor() {
		super({
			id: VoiceChatInChatViewAction.ID,
			title: localize2('workbench.action.aideChat.voiceChatInView.label', "Voice Chat in Chat View"),
...
export class HoldToVoiceChatInChatViewAction extends Action2 {
	...
	static readonly ID = 'workbench.action.aideChat.holdToVoiceChatInChatView';

	constructor() {
		super({
			id: HoldToVoiceChatInChatViewAction.ID,
			title: localize2('workbench.action.aideChat.holdToVoiceChatInChatView.label', "Hold to Voice Chat in Chat View"),
			keybinding: {
...
export class InlineVoiceChatAction extends VoiceChatWithHoldModeAction {
	...
	static readonly ID = 'workbench.action.aideChat.inlineVoiceChat';

	constructor() {
		super({
			id: InlineVoiceChatAction.ID,
			title: localize2('workbench.action.aideChat.inlineVoiceChat', "Inline Voice Chat"),
			category: CHAT_CATEGORY,
			precondition: ContextKeyExpr.and(
				CanVoiceChat,
				ActiveEditorContext,
...
export class QuickVoiceChatAction extends VoiceChatWithHoldModeAction {
	...
	static readonly ID = 'workbench.action.aideChat.quickVoiceChat';

	constructor() {
		super({
			id: QuickVoiceChatAction.ID,
			title: localize2('workbench.action.aideChat.quickVoiceChat.label', "Quick Voice Chat"),
			category: CHAT_CATEGORY,
			precondition: ContextKeyExpr.and(
				CanVoiceChat,
				CONTEXT_CHAT_REQUEST_IN_PROGRESS.negate() // disable when a chat request is in progress
...
export class StartVoiceChatAction extends Action2 {
	...
	static readonly ID = 'workbench.action.aideChat.startVoiceChat';

	constructor() {
		super({
			id: StartVoiceChatAction.ID,
			title: localize2('workbench.action.aideChat.startVoiceChat.label', "Start Voice Chat"),
			category: CHAT_CATEGORY,
			f1: true,
			keybinding: {
				weight: KeybindingWeight.WorkbenchContrib,
...
export class StopListeningAction extends Action2 {
	...
	static readonly ID = 'workbench.action.aideChat.stopListening';

	constructor() {
		super({
			id: StopListeningAction.ID,
			title: localize2('workbench.action.aideChat.stopListening.label', "Stop Listening"),
			category: CHAT_CATEGORY,
			f1: true,
			keybinding: {
...
export class StopListeningAndSubmitAction extends Action2 {
	...
	static readonly ID = 'workbench.action.aideChat.stopListeningAndSubmit';

	constructor() {
		super({
			id: StopListeningAndSubmitAction.ID,
			title: localize2('workbench.action.aideChat.stopListeningAndSubmit.label', "Stop Listening and Submit"),
			category: CHAT_CATEGORY,
			f1: true,
			keybinding: {
				weight: KeybindingWeight.WorkbenchContrib,
...
class ChatSynthesizerSessionController {
	...
	private static doCreateForFocusedChat(accessor: ServicesAccessor, response: IChatResponseModel): IChatSynthesizerSessionController {
		...
		if (chatWidget?.location === AideChatAgentLocation.Editor) {
...
export class ReadChatResponseAloud extends Action2 {
	constructor() {
		super({
			id: 'workbench.action.aideChat.readChatResponseAloud',
			title: localize2('workbench.action.aideChat.readChatResponseAloud', "Read Aloud"),
			icon: Codicon.unmute,
			precondition: CanVoiceChat,
			menu: {
				id: MenuId.AideChatMessageTitle,
				when: ContextKeyExpr.and(
					CanVoiceChat,
...
export class StopReadChatItemAloud extends Action2 {
	...
	static readonly ID = 'workbench.action.aideChat.stopReadChatItemAloud';

	constructor() {
		super({
			id: StopReadChatItemAloud.ID,
			icon: Codicon.mute,
			title: localize2('workbench.action.aideChat.stopReadChatItemAloud', "Stop Reading Aloud"),
...
function supportsKeywordActivation(configurationService: IConfigurationService, speechService: ISpeechService, chatAgentService: IAideChatAgentService): boolean {
	if (!speechService.hasSpeechProvider || !chatAgentService.getDefaultAgent(AideChatAgentLocation.Panel)) {
		return false;
	}
...
export class KeywordActivationContribution extends Disposable implements IWorkbenchContribution {
	...
	private updateConfiguration(): void {
		if (!this.speechService.hasSpeechProvider || !this.chatAgentService.getDefaultAgent(AideChatAgentLocation.Panel)) {
...
export class InstallSpeechProviderForVoiceChatAction extends BaseInstallSpeechProviderAction {
	...
	static readonly ID = 'workbench.action.aideChat.installProviderForVoiceChat';

	constructor() {
		super({
			id: InstallSpeechProviderForVoiceChatAction.ID,
			title: localize2('workbench.action.aideChat.installProviderForVoiceChat.label', "Start Voice Chat"),
			icon: Codicon.mic,
...
export class InstallSpeechProviderForSynthesizeChatAction extends BaseInstallSpeechProviderAction {
	...
	static readonly ID = 'workbench.action.aideChat.installProviderForSynthesis';

	constructor() {
		super({
			id: InstallSpeechProviderForSynthesizeChatAction.ID,
			title: localize2('workbench.action.aideChat.installProviderForSynthesis.label', "Read Aloud"),
			icon: Codicon.unmute,
			precondition: InstallingSpeechProvider.negate(),
			menu: [{
				id: MenuId.AideChatMessageTitle,
...
registerThemingParticipant((theme, collector) => {
	...
	collector.addRule(`
		.monaco-workbench:not(.reduce-motion) .cschat-input-part .monaco-action-bar .action-label.codicon-sync.codicon-modifier-spin:not(.disabled),
		...
		.monaco-workbench:not(.reduce-motion) .cschat-input-part .monaco-action-bar .action-label.codicon-sync.codicon-modifier-spin:not(.disabled)::before,
		...
		.monaco-workbench:not(.reduce-motion) .cschat-input-part .monaco-action-bar .action-label.codicon-sync.codicon-modifier-spin:not(.disabled)::after,
