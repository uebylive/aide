...
export class QuickChatService extends Disposable implements IQuickChatService {
	...
	get enabled(): boolean {
		return !!this.chatService.isEnabled(AideChatAgentLocation.Panel);
	}
...
class QuickChat extends Disposable {
	...
	render(parent: HTMLElement): void {
		...
		this.widget = this._register(
			scopedInstantiationService.createInstance(
				...
				{ resource: true },
				{ renderInputOnTop: true, renderStyle: 'compact', menus: { inputSideToolbar: MenuId.AideChatInputSide } },
				{
	...
	async openChatView(): Promise<void> {
		...
		for (const request of this.model.getRequests()) {
			if (request.response?.response.value || request.response?.result) {
				...
				const message: IAideChatProgress[] = [];
	...
	private updateModel(): void {
		this.model ??= this.chatService.startSession(AideChatAgentLocation.Panel, CancellationToken.None);
		if (!this.model) {
