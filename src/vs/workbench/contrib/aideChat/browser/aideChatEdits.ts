...
export const IAideChatEditSessionService = createDecorator<IAideChatEditSessionService>('aideChatEditSessionService');

export interface IAideChatEditSessionService {
	...
	sendEditRequest(responseVM: IChatResponseViewModel, request: IAideChatAgentEditRequest): Promise<{ responseCompletePromise: Promise<void> } | undefined>;
...
export class AideChatEditSessionService extends Disposable implements IAideChatEditSessionService {
	...
	private _receivedProgress = new Map<string, IAideChatAgentEditResponse[]>();
	...
	constructor(
		...
		@IAideChatAgentService protected readonly csChatAgentService: IAideChatAgentService,
	) {
		super();
	...
	async sendEditRequest(responseVM: IChatResponseViewModel, request: IAideChatAgentEditRequest): Promise<{ responseCompletePromise: Promise<void> } | undefined> {
	...
	private async _sendEditRequestAsync(responseVM: IChatResponseViewModel, request: IAideChatAgentEditRequest): Promise<void> {
		const progressiveEditsAvgDuration = new MovingAverage();
		const progressiveEditsClock = StopWatch.create();
		const progressiveEditsQueue = new Queue();

		const rawResponsePromise = createCancelablePromise<void>(async token => {
			const progressCallback = async (progress: IAideChatAgentEditResponse) => {
	...
	private async _makeChanges(response: IChatResponseViewModel, progress: IAideChatAgentEditResponse, opts: ProgressingEditsOptions | undefined) {
...
export class LiveStrategy extends EditModeStrategy {
	...
	protected _updateSummaryMessage(uri: URI, diff: IDocumentDiff | null) {
		...
		const editSummary: IAideChatEditSummary = {
			location: { uri, range },
			summary: message
		};
