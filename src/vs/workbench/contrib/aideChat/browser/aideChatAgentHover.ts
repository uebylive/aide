...
import { getFullyQualifiedId, IChatAgentData, IAideChatAgentNameService, IAideChatAgentService } from '../../../../workbench/contrib/aideChat/common/aideChatAgents.js';
import { showExtensionsWithIdsCommandId } from '../../../../workbench/contrib/extensions/browser/extensionsActions.js';
import { verifiedPublisherIcon } from '../../../../workbench/contrib/extensions/browser/extensionsIcons.js';
...
export class ChatAgentHover extends Disposable {
	...
	setAgent(id: string): void {
		...
		} else if (agent.metadata.themeIcon instanceof URI) {
			const avatarIcon = dom.$<HTMLImageElement>('img.icon');
			avatarIcon.src = FileAccess.uriToBrowserUri(agent.metadata.themeIcon).toString(true);
