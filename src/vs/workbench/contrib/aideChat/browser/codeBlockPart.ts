	...
	private async updateEditor(data: ICodeCompareBlockData, token: CancellationToken): Promise<void> {
		...
		if (data.edit.state?.applied) {
			...
			if (data.edit.state.applied === 1) {
				template = localize('aideChat.edits.1', "Made 1 change in [[``{0}``]]", uriLabel);
			} else if (data.edit.state.applied < 0) {
				template = localize('aideChat.edits.rejected', "Edits in [[``{0}``]] have been rejected", uriLabel);
			} else {
				template = localize('aideChat.edits.N', "Made {0} changes in [[``{1}``]]", data.edit.state.applied, uriLabel);
			}
