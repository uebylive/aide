...
import { CTX_INLINE_CHAT_HAS_AGENT, EditMode } from '../../../../workbench/contrib/inlineAideChat/common/inlineChat.js';
import { IEditorService } from '../../../../workbench/services/editor/common/editorService.js';
import { UntitledTextEditorInput } from '../../../../workbench/services/untitled/common/untitledTextEditorInput.js';
import { EmptyResponse, ErrorResponse, HunkData, ReplyResponse, Session, SessionExchange, SessionWholeRange, StashedSession, TelemetryData, TelemetryDataClassification } from './inlineChatSession.js';
import { IInlineChatSessionEndEvent, IInlineChatSessionEvent, IInlineAideChatSessionService, ISessionKeyComputer, Recording } from './inlineChatSessionService.js';
...
export class InlineChatSessionServiceImpl implements IInlineAideChatSessionService {
	...
	constructor(
		...
		@IAideChatVariablesService chatVariableService: IAideChatVariablesService,
	) {
	...
	async createSession(editor: IActiveCodeEditor, options: { editMode: EditMode; wholeRange?: Range }, token: CancellationToken): Promise<Session | undefined> {
		...
		const agent = this._chatAgentService.getDefaultAgent(AideChatAgentLocation.Editor);
		...
		const chatModel = this._chatService.startSession(AideChatAgentLocation.Editor, token);
		if (!chatModel) {
...
export class InlineChatEnabler {
	...
	constructor(
		...
		this._store.add(chatAgentService.onDidChangeAgents(() => {
			const hasEditorAgent = Boolean(chatAgentService.getDefaultAgent(AideChatAgentLocation.Editor));
