...
class MainThreadChatTask implements IAideChatTask {
...
export class MainThreadChatAgents2 extends Disposable implements MainThreadAideChatAgentsShape2 {
	...
	private readonly _pendingProgress = new Map<string, (part: IAideChatProgress) => void>();
	private readonly _proxy: ExtHostAideChatAgentsShape2;

	private _responsePartHandlePool = 0;
	private readonly _activeTasks = new Map<string, IAideChatTask>();
	...
	$registerAgent(handle: number, extension: ExtensionIdentifier, id: string, metadata: IExtensionChatAgentMetadata, dynamicProps: IDynamicChatAgentProps | undefined): void {
		...
		const impl: IChatAgentImplementation = {
			...
			provideFollowups: async (request, result, history, token): Promise<IAideChatFollowup[]> => {
			...
			provideWelcomeMessage: (location: AideChatAgentLocation, token: CancellationToken) => {
				return this._proxy.$provideWelcomeMessage(handle, location, token);
			},
			provideSampleQuestions: (location: AideChatAgentLocation, token: CancellationToken) => {
			...
			disposable = this._chatAgentService.registerDynamicAgent(
				{
					...
					slashCommands: [],
					locations: [AideChatAgentLocation.Panel] // TODO all dynamic participants are panel only?
				},
				impl);
		} else {
	...
	async $handleProgressChunk(requestId: string, progress: IChatProgressDto, responsePartHandle?: number): Promise<number | void> {
		const revivedProgress = revive(progress) as IAideChatProgress;
