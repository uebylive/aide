...
import { ExtHostAIModelsShape, ExtHostContext, MainContext, MainThreadAIModelsShape } from 'vs/workbench/api/common/extHost.protocol';
import { IAIModelStatsService } from 'vs/workbench/contrib/aideChat/common/languageModelStats';
import { IAIModelChatMetadata, IAideChatResponseFragment, IAIModelsService, IAideChatMessage, ILanguageModelAideChatSelector } from 'vs/workbench/contrib/aideChat/common/languageModels';
...
@extHostNamedCustomer(MainContext.MainThreadAIModels)
export class MainThreadAIModels implements MainThreadAIModelsShape {

	private readonly _proxy: ExtHostAIModelsShape;
	private readonly _store = new DisposableStore();
	private readonly _providerRegistrations = new DisposableMap<number>();
	private readonly _pendingProgress = new Map<number, IProgress<IAideChatResponseFragment>>();

	constructor(
		extHostContext: IExtHostContext,
		@IAIModelsService private readonly _chatProviderService: IAIModelsService,
		@IAIModelStatsService private readonly _languageModelStatsService: IAIModelStatsService,
		...
		this._proxy = extHostContext.getProxy(ExtHostContext.ExtHostAideChatProvider);
	...
	async $handleProgressChunk(requestId: number, chunk: IAideChatResponseFragment): Promise<void> {
	...
	async $fetchResponse(extension: ExtensionIdentifier, providerId: string, requestId: number, messages: IAideChatMessage[], options: {}, token: CancellationToken): Promise<any> {
