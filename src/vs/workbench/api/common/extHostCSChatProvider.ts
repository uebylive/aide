...
import { ICSChatMessage, ICSChatResponseFragment } from 'vs/workbench/contrib/csChat/common/csChatProvider';
...
class ChatRequest {
	...
	handleFragment(fragment: ICSChatResponseFragment): void {
		if (this._isDone) {
			return;
		}
...
export class ExtHostCSChatProvider implements ExtHostCSChatProviderShape {
	...
	registerProvider(extension: ExtensionIdentifier, identifier: string, provider: vscode.ChatResponseProvider, metadata: vscode.ChatResponseProviderMetadata): IDisposable {
		...
		const handle = ExtHostCSChatProvider._idPool++;
	...
	async $provideChatResponse(handle: number, requestId: number, messages: ICSChatMessage[], options: { [name: string]: any }, token: CancellationToken): Promise<any> {
		...
		return data.provider.provideChatResponse(messages.map(typeConvert.CSChatMessage.to), options, progress, token);
	}
	...
	async $handleResponseFragment(requestId: number, chunk: ICSChatResponseFragment): Promise<void> {
