...
export class ExtHostCSChat implements ExtHostCSChatShape {
	...
	constructor(
		...
		this._proxy = mainContext.getProxy(MainContext.MainThreadCSChat);
	}

	//#region CSChat session

	registerChatProvider(extension: Readonly<IRelaxedExtensionDescription>, id: string, provider: vscode.CSChatSessionProvider): vscode.Disposable {
	...
	sendCSChatRequestToProvider(providerId: string, message: vscode.CSChatSessionDynamicRequest): void {
	...
	async $provideReply(handle: number, sessionId: number, request: IChatRequestDto, token: CancellationToken): Promise<IChatResponseDto | undefined> {
		...
		const progressObj: vscode.Progress<vscode.CSChatProgress> = {
			report: (progress: vscode.CSChatProgress) => {
		...
		let result: vscode.CSChatResponseForProgress | undefined | null;
		try {
