...
import { ExtHostAIModelsShape, MainContext, MainThreadAIModelsShape } from 'vs/workbench/api/common/extHost.protocol';
import { IExtHostAuthentication } from 'vs/workbench/api/common/extHostAuthentication';
import { IExtHostRpcService } from 'vs/workbench/api/common/extHostRpcService';
import * as typeConvert from 'vs/workbench/api/common/extHostTypeConverters';
import * as extHostTypes from 'vs/workbench/api/common/extHostTypes';
import { IAideChatMessage, IAideChatResponseFragment, IAIModelChatMetadata } from 'vs/workbench/contrib/aideChat/common/languageModels';
...
class LanguageModelResponse {
	...
	handleFragment(fragment: IAideChatResponseFragment): void {
		if (this._isDone) {
			return;
		}
...
export class ExtHostAIModels implements ExtHostAIModelsShape {
	...
	private readonly _proxy: MainThreadAIModelsShape;
	...
	private readonly _allLanguageModelData = new Map<string, { metadata: IAIModelChatMetadata; apiObjects: ExtensionIdentifierMap<vscode.LanguageModelChat> }>(); // these are ALL models, not just the one in this EH
	...
	constructor(
		...
		this._proxy = extHostRpc.getProxy(MainContext.MainThreadAIModels);
	}
	...
	async $provideLanguageModelResponse(handle: number, requestId: number, from: ExtensionIdentifier, messages: IAideChatMessage[], options: { [name: string]: any }, token: CancellationToken): Promise<any> {
		...
		return data.provider.provideLanguageModelResponse(
			messages.map(typeConvert.AIModelChatMessage.to),
			options,
	...
	$acceptChatModelMetadata(data: { added?: { identifier: string; metadata: IAIModelChatMetadata }[] | undefined; removed?: string[] | undefined }): void {
		if (data.added) {
				...
				apiObject = {
					...
					countTokens(text, token) {
						if (!that._allLanguageModelData.has(identifier)) {
							throw extHostTypes.AIModelError.NotFound(identifier);
						}
	...
	private async _sendChatRequest(extension: IExtensionDescription, languageModelId: string, messages: vscode.LanguageModelChatMessage[], options: vscode.LanguageModelChatRequestOptions, token: CancellationToken) {
		...
		const internalMessages: IAideChatMessage[] = this._convertMessages(extension, messages);
		...
		if (!metadata || !this._allLanguageModelData.has(languageModelId)) {
			throw extHostTypes.AIModelError.NotFound(`Language model '${languageModelId}' is unknown.`);
		}
		...
		if (this._isUsingAuth(from, metadata)) {
			...
			if (!success || !this._modelAccessList.get(from)?.has(metadata.extension)) {
				throw extHostTypes.AIModelError.NoPermissions(`Language model '${languageModelId}' cannot be used by '${from.value}'.`);
			}
		}
		...
		if (error) {
			if (error.name === extHostTypes.AIModelError.name) {
				throw error;
			}

			throw new extHostTypes.AIModelError(
	...
	private _convertMessages(extension: IExtensionDescription, messages: vscode.LanguageModelChatMessage[]) {
		const internalMessages: IAideChatMessage[] = [];
		for (const message of messages) {
			...
			internalMessages.push(typeConvert.AIModelChatMessage.from(message));
		}
		return internalMessages;
	}

	async $handleResponseFragment(requestId: number, chunk: IAideChatResponseFragment): Promise<void> {
	...
	private _isUsingAuth(from: ExtensionIdentifier, toMetadata: IAIModelChatMetadata): toMetadata is IAIModelChatMetadata & { auth: NonNullable<IAIModelChatMetadata['auth']> } {
	...
	private async _fakeAuthPopulate(metadata: IAIModelChatMetadata): Promise<void> {
	...
	private async _computeTokenLength(languageModelId: string, value: string | vscode.LanguageModelChatMessage, token: vscode.CancellationToken): Promise<number> {
		...
		return this._proxy.$countTokens(languageModelId, (typeof value === 'string' ? value : typeConvert.AIModelChatMessage.from(value)), token);
	}
	...
	createLanguageModelAccessInformation(from: Readonly<IExtensionDescription>): vscode.LanguageModelAccessInformation {
		...
		return {
			...
			canSendRequest(chat: vscode.LanguageModelChat): boolean | undefined {
				...
				let metadata: IAIModelChatMetadata | undefined;
