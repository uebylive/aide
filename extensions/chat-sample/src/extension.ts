...
const DEFAULT_CAT_PARTICIPANT_ID = 'chat-sample.alphaCat';
...
export function activate(context: vscode.ExtensionContext) {
	...
	const defaultHandler: vscode.ChatRequestHandler = async (request: vscode.ChatRequest, context: vscode.ChatContext, stream: vscode.ChatResponseStream, token: vscode.CancellationToken): Promise<ICatChatResult> => {
		stream.progress('Yo!');
		return {
			metadata: {
				command: 'default'
	...
	const defaultCat = vscode.chat.createChatParticipant(DEFAULT_CAT_PARTICIPANT_ID, defaultHandler);
