...
export const Card = ({ cardContext, eventType, timestamp, children }: CardProps) => {
  ...
  return (
    <div
      ...
      <div className="border-4 border-cs-bgPrimary text-base rounded-b">{children}</div>
      <p className="text-xs text-right mt-2">{makeTimestampHumanReadable(timestamp)}</p>
    </div>
  );
};
