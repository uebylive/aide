...
const getOriginalPrompt = (antonData: AntonData) => {
  // Find the first event with event_type === "initial_thinking" and return its event_input.
  const initialThinkingEvent = antonData.events.find(
    (event) => event.event_type === "initial_thinking"
  );
  return initialThinkingEvent?.event_input ?? "";
};
...
const sortEvents = (antonData: AntonData) => {
  const sortedEvents = [...antonData.events].sort((a, b) => {
    if (a.execution_event_id === null && b.execution_event_id !== null) {
      return -1;
    } else if (a.execution_event_id !== null && b.execution_event_id === null) {
      return 1;
    } else if (a.execution_event_id === null && b.execution_event_id === null) {
      return a.event_timestamp - b.event_timestamp;
    } else {
      return a.execution_event_id! - b.execution_event_id!;
    }
  });
...
// `event_type` = `exploring_node_dfs` and `event_context` = `dfs_start`
...
const fixReferenceLookupOrder = (antonData: AntonData) => {
  ...
  const dfsStartEvents = events.filter(
    (event) => event.event_type === "exploring_node_dfs" && event.event_context === "dfs_start"
  );
  const referenceLookupEvents = events.filter(
    (event) => event.event_type === "get_references_for_code_node"
  );
  const dfsStartEventTimestamps = dfsStartEvents.map((event) => event.event_timestamp);
  ...
  dfsStartEventTimestampsToFix.forEach((timestamp) => {
    const dfsStartEventIndex = events.findIndex(
      (event) =>
        event.event_type === "exploring_node_dfs" &&
        event.event_context === "dfs_start" &&
        event.event_timestamp === timestamp
    );
    const referenceLookupEventIndex = events.findIndex(
      (event) =>
        event.event_type === "get_references_for_code_node" && event.event_timestamp === timestamp
    );
...
// Function to filter AntonData `events` with `event_type` as `exploring_node_dfs`.
// Each of these nodes will have a `event_context` key and a single entry in the `code_symbol_reference` array.
// Return a single event with `event_type` as `exploring_node_dfs` where the `code_symbol_reference` array
...
const coalesceNodeExplorationEvets = (antonData: AntonData) => {
  const exploringNodeDfsEvents = antonData.events.filter(
    (event) => event.event_type === "exploring_node_dfs"
  );
  const codeSymbolReference = exploringNodeDfsEvents.reduce(
    (acc, event) => [
      ...(acc ?? []),
      ...(event.code_symbol_reference ?? []).map((e) => ({
        ...e,
        event_context: event.event_context ?? "",
      })),
    ],
    [] as AntonData["events"][0]["code_symbol_reference"]
  );
...
const processAntonData = (antonData: AntonData): AntonData => {
  const filteredEvents = antonData.events.filter(
    (event) =>
      event.event_type !== "get_references_for_code_node" && event.event_type !== "initial_thinking"
  );
    ...
    messageHandler
        ...
        interval = setInterval(() => {
          ...
          if (event) {
            setAntonData((prevData) => {
              return {
                save_destination: prevData?.save_destination ?? "",
      ...
      .catch((err) => {
        ...
        if (process.env.NODE_ENV === "development") {
          import("../mocks/sample.json").then((sampleData) => {
