	...
	const renderCardContents = () => {
		switch (data.eventType) {
			...
			case 'codeSymbolModificationInstruction':
				return (
					<div className='p-4'>
						<Breadcrumbs
							path={
								data.codeSymbolModificationInstruction?.codeSymbolName.split(
									'.'
								) ?? []
							}
						/>
						<Markdown
							children={
								data.codeSymbolModificationInstruction?.instructions ?? ''
							}
						/>
					</div>
				);
			case 'codeSymbolModificationEvent':
				return (
					<div className='p-4'>
						<Markdown
							children={
								data.codeModificationContextAndDiff?.codeModification ?? ''
							}
						/>
						<DiffView
							className='pt-4'
			...
			case 'testExecutionHarness':
				return (
					<Markdown
						className='p-4'
						children={
							data.testExecutionHarness?.planForTestScriptGeneration ?? ''
						}
					/>
				);
			...
			case 'executionBranchFinishReason':
				return (
					<div className='p-4'>
						Exploration {data.executionEventId ? data.executionEventId + 1 : ''}{' '}
						ended with `{data.executionBranchFinishReason}`
					</div>
				);
