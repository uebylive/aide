...
export const promptClassifier = async (userMessage: string): Promise<UserMessageType> => {
	const completion = await OpenAIChat.run({
		model: 'gpt-3.5-turbo',
		messages: [
			{
				role: 'system',
	...
	if (completion.choices.length === 0) {
		...
		if (completion.choices[0].message.content === 'explain') {
			return 'explain';
		} else if (completion.choices[0].message.content === 'general') {
			return 'general';
		} else if (completion.choices[0].message.content === 'instruction') {
			return 'instruction';
		} else if (completion.choices[0].message.content === 'search') {
			return 'search';
		} else if (completion.choices[0].message.content === 'help') {
			return 'help';
		}
	}
	return 'general';
