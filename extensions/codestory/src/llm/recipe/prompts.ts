...
export function generatePlanAndQueriesResponseParser(
    ...
    let additionalInstructions = response
    ...
    let additionalQueries = response.split("<queries>")[1].split("</queries>")[0];
...
export const parseCodeModificationResponse = (
    ...
    let codePlanning = response.split("<code_planning>")[1].split("</code_planning>")[0];
    const codePlanningList = codePlanning.split("\n*");
    const codePlanningInstructions = codePlanningList
        .map((instruction) => instruction.trim())
        .filter((instruction) => instruction.length > 0);

    let codeGeneration = response.split("<code_generation>")[1].split("</code_generation>")[0];
