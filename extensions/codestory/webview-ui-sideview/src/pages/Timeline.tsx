...
const groupChangedCodeSymbols = (changes: CodeSymbolChange[]): GroupedChangesState => {
 ...
 *
 * @return {JSX.Element} The timeline component.
 */
export const TimeLine = (): JSX.Element => {
  ...
  const { codeBlockChangeDescriptions } = useChangedCodeBlockChangeDescriptionStore();
  const [componentIdentifierToChange, setComponentIdentifierToChange] = useState<
    Map<string, ChangeDescription>
  >(getComponentIdentifierToChange(codeBlockChangeDescriptions));

  useEffect(() => {
    setComponentIdentifierToChange(getComponentIdentifierToChange(codeBlockChangeDescriptions));
  ...
  const toggleGroupForCommit: React.MouseEventHandler<HTMLInputElement> = (e) => {
    e.stopPropagation();
    e.preventDefault();
  ...
  const initiateCommit = () => {
    const commitPrepData: CommitPrepData = { changedFiles: [], changeDescriptions: [] };
      ...
      if (value.selection.selected) {
        Object.entries(value.files).forEach(([filePath]) => {
          ...
          commitPrepData.changeDescriptions.push(
            componentIdentifierToChange.get(componentIdentifier) ?? { summary: "", changes: [] }
          );
        });
      }
    });
                  ...
                  {Object.entries(value.files).map(([key, value]) => {
                    return (
                      <div key={key} className="text-base mb-2 last:mb-0">
                        <p className="text-sm text-vscode-button-background">{key}</p>
                        <p className="text-xs text-gray-500">
                          {getRelativeTime(
                            value.reduce((prev, next) =>
                              prev.changeTime.getTime() < next.changeTime.getTime() ? prev : next
                            ).changeTime
                          )}
                        </p>
                        <ul>
                          {value.map((change, i) => (
                            <li key={i} className="text-vscode-sideBar-foreground">
                              <p
                                className="hover:cursor-pointer"
                                onClick={() => handleOpenFile(change.filePath, change.startLine)}>
                                <span
          ...
          {commitMode && (
            <>
              {Object.entries(groupedChanges).some(
                ([_, value]) => value.selection.reason === "STAGED AUTOMATICALLY"
              ) && (
                <p className="border-l-4 pl-2 border-vscode-foreground">
                  Some change sets are staged automatically because CodeStory only supports staging
                  entire files currently.
                </p>
              )}
              <div className="w-full flex justify-end mt-5">
                ...
                <VSCodeButton
                  ...
                  disabled={
                    !Object.entries(groupedChanges).some(([_, value]) => value.selection.selected)
                  }>
                  COMMIT
                </VSCodeButton>
              </div>
