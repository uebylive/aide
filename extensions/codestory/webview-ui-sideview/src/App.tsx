...
function App() {
  ...
  const { setCodeBlockChangeDescriptions } = useChangedCodeBlockChangeDescriptionStore();
  ...
  useEffect(() => {
    ...
    const listener = (message: MessageEvent<EventData<unknown>>) => {
      ...
      if (command === "showExplanation") {
        ...
        const payloadData = (payload as { changes: CodeSymbolChange[] }).changes;
        console.log("[debugging] What is the changelog", payloadData);
        setChangedCodeSymbol(
          payloadData.map((change) => ({ ...change, changeTime: new Date(change.changeTime) }))
        );
